	...
	<head>
        ...
        Ext.onReady(function () {
            ...
            this.store_1 = Ext.create("Ext.data.JsonStore", {
                ...
                proxy: {
                    type: "ajax",
                    url: "pubFileContactsInfo/list.do",
		 ...
		 function createForm(purl,load,id){
		    ...
		    if(load ==true,id!=null){
	    		form_1.getForm().load({
			    	url: 'pubFileContactsInfo/loadBean.do',
				    params: {
				        id: id
				    },
				    failure: function(form, action) {
		...
		//增加
		function add(){
			createForm('pubFileContactsInfo/add.do',false,null);
		}
		//编辑
		function edit(){
            ...
            if (selModel.hasSelection()) {
                  ...
                  if(selected.length==1&&selected[0].data.id!=null){
                  		createForm('pubFileContactsInfo/update.do',true,selected[0].data.id);
                  }else{
            ...
            if (selModel.hasSelection()) {
                Ext.Msg.confirm("警告", "确定要删除吗？", function (button) {
                    if (button == "yes") {
				        ...
				        Ext.Ajax.request({
				            url: 'pubFileContactsInfo/delete.do',
				            method: "post",
