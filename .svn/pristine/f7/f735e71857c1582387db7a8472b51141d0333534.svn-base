//领导交办件
Ext.define('Wdbl.form.LeaderEditForm', {
	...
	addSave : function() {
		...
		me.getForm().submit({
			...
			url : urladdress,
			params:{docTypeName:'LEADER_ASND_DOC',sessionID:me.id},
			method : "POST",
	...
	addLeaderCommand : function(){
		var me = this;
		var addFileWin = Ext.create('Wdbl.form.op.LeaderJNCommandForm',{commandContainer:me.commandContainer,removeFun:me.removeLeaderCommand});
		addFileWin.show();
	},
	// 是否显示按钮
	...
	initComponent : function() {
		...
		Ext.applyIf(me, {
			items : [{
				xtype : "fieldset",
				title : '领导交办件('+(me.barNo==null?"":me.barNo)+')',
				items : [{
				xtype : "textarea",
				fieldLabel : "文件标题",
				name : "title",
