...
<html>
	<head>
        ...
        Ext.onReady(function () {
			...
			store_1.on('beforeload', function (store1, options) {
        		var new_params = { name: s_filed_1.getValue(),property:'id',direction:'asc'};//name与PubJleaderinfo实体类中属性对应
            ...
            this.gridpanel = Ext.create("Ext.grid.Panel", {
                ...
                columns: [
                    ...
                    { header: "职务", dataIndex: "position", sortable: true ,flex : 5},
                    { header: "联系电话", dataIndex: "telephoneUnit", sortable: true ,flex : 5},
                    { header: "单位名称", dataIndex: "company", sortable: true ,flex : 5},
                ],
		 ...
		 gridpanel.setHeight(document.getElementById("pubUnitInfoGrid").offsetHeight);
		 ...
		 function createForm(purl,load,id){
		 	...
		 	var filed_job = Ext.create('Ext.form.Text', {name: "position", fieldLabel: '职务',labelAlign: 'right',allowBlank:false});
		 	var filed_phone = Ext.create('Ext.form.Text', {name: "telephoneUnit", fieldLabel: '联系电话',labelAlign: 'right',allowBlank:false});
		 	var filed_unit = Ext.create('Ext.form.Text', {name: "company", fieldLabel: '单位名称',labelAlign: 'right',allowBlank:false});
			...
			var form_1 = Ext.create('Ext.form.FormPanel',{
	            ...
	            items: [filed_id,filed_name,filed_job,filed_phone,filed_unit],
		        buttons: [{
		            text: '保存',
		            formBind:true,
		            monitorValid:true,
		...
		//增加
		function add(){
			createForm('pubExternUnitInfo/add.do',false,null);
		}
		//编辑
		function edit(){
            ...
            if (selModel.hasSelection()) {
                  ...
                  if(selected.length==1&&selected[0].data.id!=null){
                  		createForm('pubExternUnitInfo/update.do',true,selected[0].data.id);
                  }else{
	...
	<body>  
		<div id="pubUnitInfoGrid" style="height: 100%;width: 100%;"></div> 
	</body>
</html>