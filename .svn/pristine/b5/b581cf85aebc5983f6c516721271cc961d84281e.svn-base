...
import com.capinfo.pub.bean.PubProvinceleaderInfo;
...
@Controller  
@Scope("prototype")
@RequestMapping("/pubProvinceleaderInfo")
public class PubProvinceleaderInfoController {
	
	public static Logger log = Logger.getLogger(PubProvinceleaderInfoController.class);
	@Autowired
	...
	public ModelAndView toPubProvinceleaderInfoList(){
		return new ModelAndView("pub/pubProvinceleaderInfoList");
	}
	/**列表加载数据*/
	@RequestMapping(value="/list.do",method = RequestMethod.POST)  
	@ResponseBody 
    public Object list(PageBean page,PubProvinceleaderInfo p){
		...
		try{
			...
			List<PubProvinceleaderInfo> list = baseService.findByPage(page, PubProvinceleaderInfo.class, searchMap, orderMap);
	...
	@ResponseBody 
    public Object addSave(PubProvinceleaderInfo t1){
	...
	@ResponseBody 
    public Object editSave(PubProvinceleaderInfo t){
		SaveJsonBean result = new SaveJsonBean();
		try{
    ...
    public Object loadBean(@RequestParam(value="id",required=true) Long id){
		FormLoadJsonBean<PubProvinceleaderInfo> result = new FormLoadJsonBean<PubProvinceleaderInfo>();
		try{
			PubProvinceleaderInfo bean = baseService.getObject(PubProvinceleaderInfo.class, id);
			result.setSuccess(true);
			result.setData(bean);
		}catch(Exception e){
			e.printStackTrace();
    ...
    public Object delete(long[] ids){
		...
		try{
			if(ids!=null&&ids.length>0){
				Collection<PubProvinceleaderInfo> list=new ArrayList<PubProvinceleaderInfo>(); 
				for(int i=0;i<ids.length;i++){
					PubProvinceleaderInfo t = new PubProvinceleaderInfo();
					t.setId(ids[i]);
					list.add(t);
				}
