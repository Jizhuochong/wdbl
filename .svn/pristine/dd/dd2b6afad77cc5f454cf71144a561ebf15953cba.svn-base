...
import com.capinfo.pub.bean.PubWdsYwInfo;
...
@Controller  
@Scope("prototype")
@RequestMapping("/pubWdsYwInfo")
public class PubWdsYwInfoController {
	public static Logger log = Logger.getLogger(PubWdsYwInfoController.class);
	@Autowired
	...
	public ModelAndView toPubWdsYwInfoList(){
		return new ModelAndView("pub/pubWdsYwInfoList");
	}
	/**列表加载数据*/
	@RequestMapping(value="/list.do",method = RequestMethod.POST)  
	@ResponseBody 
    public Object list(PageBean page,PubWdsYwInfo p){
		GridJsonBean grid = null;
		try{
			searchMap.put("fileName",p.getFileName());//fileName对应PubWdsYwInfo实体类属性
			...
			List<PubWdsYwInfo> list = baseService.findByPage(page, PubWdsYwInfo.class, searchMap, orderMap);
	...
	@ResponseBody 
    public Object addSave(PubWdsYwInfo t1){
	...
	@ResponseBody 
    public Object editSave(PubWdsYwInfo t){
		SaveJsonBean result = new SaveJsonBean();
		try{
    ...
    public Object loadBean(@RequestParam(value="id",required=true) Long id){
		FormLoadJsonBean<PubWdsYwInfo> result = new FormLoadJsonBean<PubWdsYwInfo>();
		try{
			PubWdsYwInfo bean = baseService.getObject(PubWdsYwInfo.class, id);
			result.setSuccess(true);
			result.setData(bean);
		}catch(Exception e){
			e.printStackTrace();
    ...
    public Object delete(long[] ids){
		...
		try{
			if(ids!=null&&ids.length>0){
				Collection<PubWdsYwInfo> list=new ArrayList<PubWdsYwInfo>(); 
				for(int i=0;i<ids.length;i++){
					PubWdsYwInfo t = new PubWdsYwInfo();
					t.setId(ids[i]);
					list.add(t);
				}
