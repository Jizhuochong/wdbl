...
<html>
	<head>
        ...
        Ext.onReady(function () {
			...
			store_1.on('beforeload', function (store1, options) {
        		var new_params = { name: s_filed_1.getValue(),property:'id',direction:'asc'};//name与pubZbsNamebook实体类中属性对应
		 ...
		 gridpanel.setHeight(document.getElementById("pubZbsNamebookGrid").offsetHeight);
		 ...
		 function createForm(purl,load,id){
			...
			var form_1 = Ext.create('Ext.form.FormPanel',{
	            ...
	            items: [filed_id,filed_name,filed_job,filed_phone,filed_address],
		        buttons: [{
		            text: '保存',
		            formBind:true,
		            monitorValid:true,
		    ...
		    if(load ==true,id!=null){
	    		form_1.getForm().load({
			    	url: 'pubZbsNamebook/loadBean.do',
				    params: {
				        id: id
				    },
				    failure: function(form, action) {
		...
		//增加
		function add(){
			createForm('pubZbsNamebook/add.do',false,null);
		}
		//编辑
		function edit(){
            ...
            if (selModel.hasSelection()) {
                  ...
                  if(selected.length==1&&selected[0].data.id!=null){
                  		createForm('pubZbsNamebook/update.do',true,selected[0].data.id);
                  }else{
	...
	<body>  
		<div id="pubZbsNamebookGrid" style="height: 100%;width: 100%;"></div> 
	</body>
</html>