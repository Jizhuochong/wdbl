 ...
 */
var leaderSn = null;
Ext.define('Wdbl.form.MainLearderEditForm', {
	...
	addSave : function() {
		...
		me.getForm().submit({
			...
			url : urladdress,
			params:{docTypeName:'MAIN_LEARDER_DOC',sessionID:me.id},
			method : "POST",
			success : function(form, action) {
				me.modelId = action.result.data.id;
				leaderSn.setValue(action.result.data.fileNumber);
	...
	initComponent : function() {
		...
		leaderSn = Ext.create('Ext.form.field.Text',{
			columnWidth : 0.25,
			fieldLabel : "领导文号",
			value:"﹝"+date+"﹞",
			labelWidth:80,
		...
		var baseInfoSet = {
			xtype:'fieldset',
			title: '市局主要领导文件('+(me.barNo==null?"":me.barNo)+')',
			items : [{
				xtype : "textarea",
				fieldLabel : "文件标题",
				name : "title",
