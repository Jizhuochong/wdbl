...
import com.capinfo.pub.bean.PubJzbInfo;
...
@Controller  
@Scope("prototype")
@RequestMapping("/pubJzbInfo")
public class PubJzbInfoController {
	public static Logger log = Logger.getLogger(PubJzbInfoController.class);
	@Autowired
	...
	public ModelAndView toPubJzbInfoList(){
		return new ModelAndView("pub/pubJzbInfoList");
	}
	/**列表加载数据*/
	@RequestMapping(value="/list.do",method = RequestMethod.POST)  
	@ResponseBody 
    public Object list(PageBean page,PubJzbInfo p){
		...
		try{
			...
			List<PubJzbInfo> list = baseService.findByPage(page, PubJzbInfo.class, searchMap, orderMap);
	...
	@ResponseBody 
    public Object addSave(PubJzbInfo t1){
	...
	@ResponseBody 
    public Object editSave(PubJzbInfo t){
		SaveJsonBean result = new SaveJsonBean();
		try{
    ...
    public Object loadBean(@RequestParam(value="id",required=true) Long id){
		FormLoadJsonBean<PubJzbInfo> result = new FormLoadJsonBean<PubJzbInfo>();
		try{
			PubJzbInfo bean = baseService.getObject(PubJzbInfo.class, id);
			result.setSuccess(true);
			result.setData(bean);
		}catch(Exception e){
			e.printStackTrace();
    ...
    public Object delete(long[] ids){
		...
		try{
			if(ids!=null&&ids.length>0){
				Collection<PubJzbInfo> list=new ArrayList<PubJzbInfo>(); 
				for(int i=0;i<ids.length;i++){
					PubJzbInfo t = new PubJzbInfo();
					t.setId(ids[i]);
					list.add(t);
				}
